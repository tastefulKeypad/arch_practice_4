...
try {
    const badCars = db.cars.insertMany([
        // carClass < 1
        {
            carClass: -1,
            price: 159,
    ...
    print('Added ${Object.keys(badCars).length} bad cars');
} catch(e) {
    print('CATCH ERR: ${e.message}');
}

try {
    const badUsers = db.users.insertMany([
        // email is NOT unique
        {
            email: "admin@example.com",
            name: "Admin",
    ...
    print('Added ${Object.keys(badUsers).length} bad users');
} catch(e) {
    print('CATCH ERR: ${e.message}');
}
    
try {
    const badRents = db.rents.insertMany([
        // status length < 1
        {
            carId: carsIds[5],
            userId: usersIds[7],
    ...
    print('Added ${Object.keys(badRents).length} bad rents');
} catch(e) {
    print('CATCH ERR: ${e.message}');
}
